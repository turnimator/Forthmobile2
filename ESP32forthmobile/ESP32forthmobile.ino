...
# define OPTIONAL_I2C_SUPPORT \
  ...
  X("Wire.lastError", WIRE_LAST_ERROR, PUSH Wire.getWriteError()) \
  X("Wire.getErrorText", WIRE_GET_ERROR_TEXT, PUSH Wire.getWriteError()) \
  ...
  X("Wire.busy", WIRE_BUSY, PUSH ! Wire.available()) \
...
#define NUM_VL530X 3 // Number of VL53L0X sensors attached to system
  ...
  void setuplaser()
  {
    ...
    for (int i = 0; i < NUM_VL530X; i++) {
      ...
      delay(500); // give time to wake up.
  ...
  int readlaser0()
  {
    return sensors[0].psensor->readRange();
  }


  int readlaser1()
  {
    return sensors[1].psensor->readRange();
  }

  int readlaser2()
  {
    return sensors[2].psensor->readRange();
  }
...
#include <unistd.h>
  ...
  static void forth_init(int argc, char *argv[], void *heap,
    ...
    *g_sys.heap++ = (cell_t) forth;
    for (int i = 0; i < VOCABULARY_DEPTH; ++i) {
      *g_sys.heap++ = 0;
    }
